...
def test_get_s3_object():
    ...
    mock_s3_client.get_object.return_value = {"Body": io.BytesIO(b"student_id,name,email\n1234,John Doe,john@email.com\n")}
...
def test_obfuscate_csv(mocker):
    mock_s3_client = MagicMock()
    mock_s3_client.get_object.return_value = {"Body": io.BytesIO(b"student_id,name,email\n1234,John Doe,john@email.com\n")}
    mocker.patch("s3_csv_obfuscation.boto3.client", return_value=mock_s3_client)
